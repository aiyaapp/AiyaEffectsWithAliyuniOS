...
- (UIButton *)envButton{
    if (!_envButton) {
        ...
        [button setTitle:[@"note_city_Singapore" localString] forState:UIControlStateNormal];
...
- (void)env{
    ...
    if (_envButton.selected) {
        [AlivcDefine AlivcAppServerSetTestEnvMode:1];
    }else{
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.isChangedRow == NO) {
        ...
        if (indexPath.row < self.dataArray.count) {
            ...
            switch (module.type) {
                    // 视频拍摄
                case AVC_ET_ModuleType_VideoShooting:[[AliyunMediator shared]pushWithModuleString:AliyunShortVideoModuleString_VideoShooting nav:self.navigationController];break;
                    
                    // 视频编辑
                case AVC_ET_ModuleType_VideoEdit:[[AliyunMediator shared]pushWithModuleString:AliyunShortVideoModuleString_VideoEdit nav:self.navigationController];break;
                    
                    // 视频裁剪
                case AVC_ET_ModuleType_VideoClip:[[AliyunMediator shared]pushWithModuleString:AliyunShortVideoModuleString_VideoClip nav:self.navigationController];break;
                    
                    // 魔法相机
                case AVC_ET_ModuleType_MagicCamer:[[AliyunMediator shared]pushWithModuleString:AliyunShortVideoModuleString_MagicCamera nav:self.navigationController];break;
                    
                    // 互动直播
                case AVC_ET_ModuleType_VideoLive:[self pushTargetVCWithClassString:@"AlivcCombinationListViewController"];break;
                    
                    // 直播推流
                case AVC_ET_ModuleType_PushFlow:[self pushTargetVCWithClassString:@"AlivcLivePushConfigViewController"];break;
