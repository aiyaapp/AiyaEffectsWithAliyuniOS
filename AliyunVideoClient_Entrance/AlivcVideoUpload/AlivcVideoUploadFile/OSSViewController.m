...
@interface OSSViewController ()<UITableViewDelegate,UITableViewDataSource>{
...
- (IBAction)cancelFile:(id)sender {
    ...
    NSIndexSet *indexSet=[[NSIndexSet alloc]initWithIndex:0];
    [_tableFiles reloadSections:indexSet withRowAnimation:UITableViewRowAnimationAutomatic];
    [MBProgressHUD showMessage:@"取消上传" inView:self.view];
}
...
- (IBAction)resumeFile:(id)sender {
    ...
    NSIndexSet *indexSet=[[NSIndexSet alloc]initWithIndex:0];
    [_tableFiles reloadSections:indexSet withRowAnimation:UITableViewRowAnimationAutomatic];
    [MBProgressHUD showMessage:@"恢复上传" inView:self.view];
}

- (IBAction)getList:(id)sender {
    NSLog(@"getList clicked.");
    NSString *str = [NSString stringWithFormat:@"获取文件列表，%zd",[uploader listFiles].count];
    [MBProgressHUD showMessage:str inView:self.view];
}
...
- (IBAction)stopUpload:(id)sender {
    ...
    NSIndexSet *indexSet=[[NSIndexSet alloc]initWithIndex:0];
    [_tableFiles reloadSections:indexSet withRowAnimation:UITableViewRowAnimationAutomatic];
    [MBProgressHUD showMessage:@"停止上传" inView:self.view];
}
...
- (IBAction)pauseUpload:(id)sender {
    ...
    NSIndexSet *indexSet=[[NSIndexSet alloc]initWithIndex:0];
    [_tableFiles reloadSections:indexSet withRowAnimation:UITableViewRowAnimationAutomatic];
    [MBProgressHUD showMessage:@"暂停上传" inView:self.view];
}
...
- (void)setupSubviews {
    ...
    _tableFiles = [[UITableView alloc] initWithFrame:CGRectMake(0, 400, CGRectGetWidth(self.view.frame), ScreenHeight-400-64)];
    _tableFiles.delegate = self;
    _tableFiles.dataSource = self;
    _tableFiles.estimatedRowHeight = 0;
    _tableFiles.estimatedSectionHeaderHeight = 0;
    _tableFiles.estimatedSectionFooterHeight = 0;
...
- (void)getTestSTSEndpointBucketObjectWithFinishHandler:(void(^)(void))handler {
