...
-(void)uploadFailedWithCode:(NSString *)code message:(NSString *)message {
    ...
    _label.text = [NSString stringWithFormat:@"failed"];
        });
}
...
-(void)uploadSuccessWithResult:(VodSVideoUploadResult *)result {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        _label.text = [NSString stringWithFormat:@"success"];
    });
}
...
- (void)setupSubviews {
    _label = [[UILabel alloc] initWithFrame:CGRectMake(16, 56, 260, 40)];
    _label.backgroundColor = AlivcRGB(240,240,240);
